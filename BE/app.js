...
app.patch("/payments/:id", (req, res) => {
  ...
  if (typeof state !== "number" || (state !== 0 && state !== 1)) {
    return res.status(400).json({ error: "Giá trị state không hợp lệ (chỉ nhận 0 hoặc 1)" });
  }
  if (state === 1) {
    ...
    db.query(sql, [vnDate, id], (err, result) => {
      ...
      res.json({ message: "Cập nhật trạng thái và ngày thanh toán thành công" });
    });
  } else {
    ...
    db.query(sql, [id], (err, result) => {
      ...
      if (result.affectedRows === 0)
        return res.status(404).json({ error: "Không tìm thấy giao dịch để cập nhật" });
      res.json({ message: "Cập nhật trạng thái về chưa thanh toán thành công" });
    });
  }
});
// index.js
