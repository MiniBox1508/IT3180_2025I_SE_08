  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      if (
        ...
        !singleFormData.feetype ||
        !singleFormData.amount
      ) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!row.apartment_id || !row.feetype || !row.amount) {
          setError(
            `Dòng ${i + 1}: Vui lòng điền đủ Số căn hộ, Loại phí và Số tiền.`
          );
          return;
        }
    ...
    <div className="fixed inset-0 bg-opacity-50 flex justify-center items-center z-50 animate-fade-in">
      <div
        className={`bg-white p-6 rounded-2xl shadow-2xl relative flex flex-col ${
          isEditing ? "w-full max-w-md" : "w-full max-w-5xl"
        }`}
            ...
            <div className="overflow-y-auto custom-scrollbar border border-gray-200 rounded-lg flex-1">
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-100 sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="p-3 text-xs font-bold text-gray-600 uppercase border-b w-1/4">
                      Số căn hộ
                    </th>
                    <th className="p-3 text-xs font-bold text-gray-600 uppercase border-b w-1/4">
                      Loại phí
                    </th>
                    <th className="p-3 text-xs font-bold text-gray-600 uppercase border-b w-1/5">
                      Số tiền (VNĐ)
                    </th>
                    <th className="p-3 text-xs font-bold text-gray-600 uppercase border-b w-1/5">
                      Hạn đóng
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                type="date"
                name="payment_date"
                value={singleFormData.payment_date}
                onChange={handleSingleChange}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:border-blue-500 outline-none"
              />
            </div>
    ...
    <div className="bg-white rounded-2xl shadow-md p-4 flex items-center relative overflow-hidden mb-4">
      ...
      <div className="flex-1 grid grid-cols-6 gap-4 items-center pl-8 pr-4 text-gray-800">
        ...
        <div>
          ...
          <p className="font-medium truncate" title={item.feetype}>
            {item.feetype}
          </p>
        </div>
        <div>
...
export const AccountPayment = () => {
  ...
  const handleExportExcel = async () => {
    ...
    const fileName = `DANH_SACH_CONG_NO_KE_TOAN_BLUEMOON_${new Date()
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const validRows = [];
      const errorMessages = [];
      const validApartmentIds = residents.map((r) =>
        r.apartment_id ? r.apartment_id.toLowerCase().trim() : ""
      );
        ...
        await Promise.all(
          itemsToCreate.map(async (item) => {
            const foundResident = residents.find(
              (r) =>
                r.apartment_id &&
                r.apartment_id.toLowerCase() ===
                  item.apartment_id.trim().toLowerCase()
            );

            if (!foundResident) {
              throw new Error(
                `Không tìm thấy dữ liệu cư dân cho căn hộ ${item.apartment_id}`
              );
            }
            ...
            if (!res.ok)
              throw new Error(`Lỗi khi tạo hóa đơn cho ${item.apartment_id}`);
          })
        );
  ...
  return (
    <div>
      ...
      <ConfirmationModal
        ...
        message={`Bạn có chắc chắn muốn xóa ${selectedIds.length} hóa đơn đã chọn không?`}
      />
