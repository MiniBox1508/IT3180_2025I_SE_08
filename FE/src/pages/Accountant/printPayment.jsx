...
export const PrintPayments = () => {
  ...
  useEffect(() => {
    if (location.state && location.state.data) {
      ...
      const rawList = Array.isArray(receivedData)
        ? receivedData
        : [receivedData];
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      pdf.save(
        `HOA_DON_TONG_HOP_KE_TOAN_BLUEMOON_${dayjs().format("DDMMYYYY")}.pdf`
      );
    } catch (err) {
    ...
    <div className="w-full min-h-screen bg-blue-700 flex flex-col items-center p-8 overflow-y-auto">
      ...
      <div className="flex space-x-4 mb-6 sticky top-0 z-50 bg-blue-700 w-full justify-center py-2">
        ...
        <button
          ...
          } text-white font-bold py-2 px-6 rounded-lg shadow-lg transition-colors`}
        >
          {isPrinting
            ? "Đang xử lý..."
            : `In ${groupedInvoices.length} hóa đơn`}
        </button>
      </div>
        ...
        {groupedInvoices.map((group, index) => (
          ...
          >
            ...
            <div className="flex justify-between mb-8">
              <div className="w-2/3">
                ...
                <p className="text-sm text-gray-600 mb-1">
                  Đơn vị bán hàng: Ban Quản Trị Chung Cư Blue Moon
                </p>
                <p className="text-sm text-gray-600 mb-1">
                  Địa chỉ: 23 Đường X, Phường A, Quận B, TP. Hà Nội
                </p>
                <p className="text-sm text-gray-600">
                  Điện thoại: 034 456 8976
                </p>
              </div>
              <div className="w-1/3 text-right">
                ...
                <p className="text-sm text-gray-500">
                  Ngày {dayjs().format("DD")} tháng {dayjs().format("MM")} năm{" "}
                  {dayjs().format("YYYY")}
                </p>
              </div>
            </div>
            ...
            <div className="mb-8 text-sm text-gray-700 space-y-1">
              <p>
                <span className="font-semibold">Họ tên khách hàng:</span>{" "}
                {group.resident_name || "Cư dân"}
              </p>
              <p>
              ...
              <p>
                <span className="font-semibold">Địa chỉ:</span> Căn hộ{" "}
                {group.apartment_id}, Chung cư Blue Moon
              </p>
              <p>
                <span className="font-semibold">Hình thức thanh toán:</span>{" "}
                {group.payment_form || "Tổng hợp"}
              </p>
            </div>
