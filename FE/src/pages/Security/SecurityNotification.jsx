                ...
                {currentItems.map((item, index) => (
                  <tr key={index} className="hover:bg-blue-50 h-[50px]">
                    ...
                    <td className="p-3 text-sm text-gray-700">
                      {/* Logic hiển thị: Cư dân -> Mã căn hộ, Khác -> Tên Role */}
...
const NotificationFormModal = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  const handleSubmit = () => {
    if (isEditing) {
      const dataToSend = {
        sender_name: "Công an", // Người gửi cố định là Công an
            ...
            <div className="overflow-y-auto custom-scrollbar border border-gray-200 rounded-lg flex-1">
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-100 sticky top-0 z-10 shadow-sm">
                  <tr>
                    {/* Layout cột: Người nhận 20%, Mã căn hộ 20%, Nội dung 50%, Action 10% */}
          ...
          if (rowNumber > headerRowNumber) {
            ...
            if (rawRecipient || content) {
              ...
              if (rawRecipient && content) {
                ...
                const specialRoles = ["Kế toán", "Ban quản trị", "Tất cả"];
  ...
  const handlePrintPDF = async () => {
    try {
      ...
      reader.onloadend = () => {
        ...
        doc.setFontSize(18);
        doc.text("Danh sách thông báo - Phòng an ninh", 105, 15, {
          align: "center",
        });
        ...
        doc.save("DANH_SACH_THONG_BAO_AN_NINH_BLUEMOON.pdf");
